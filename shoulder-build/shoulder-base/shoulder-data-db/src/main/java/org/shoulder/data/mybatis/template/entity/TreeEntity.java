 ...
 *      适用情况：查询为主；深度有限制；管理时以添加操作为主，查    询时逐级查看，不涉及整个树结构查询，数据几乎不会跨级移动【简单查，插入好，删除、移动带子树的麻烦，结构简单】
 *
 * 知识拓展：
 ...
 * 【闭包模型】Closure Table Model（存储信息全面，写扩散代替读扩散），引入新一张关系表， parentId、child_id、distance 记录所有节点关系；优点：可以高性能支撑任意深度查询、快速获取树结构、跳级查看等复杂查询；缺点：多一张表；更多的关系存储写入成本；节点移动时要更新所有父关系、所有子关系、所有子的父子关系（归级别更新）；
