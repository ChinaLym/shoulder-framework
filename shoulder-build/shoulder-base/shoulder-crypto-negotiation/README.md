...
**算法安全说明**：同 `HTTPS` 中密钥协商、加解密部分，甚至可更高，默认使用 `ECDH` 算法（`DH / DSA` + `ECC`）

## 🚀 使用方式

在传输的参数或返回值DTO上添加 `@Sensitive` 注解，即可在通过 `RestTemplate` 发起服务端调用时自动实现安全传输，可以通过抓包工具校验。

### 💻 代码示例

完整工程详见 [demo5](https://gitee.com/ChinaLym/shoulder-framework-demo)
...
**出口海外** 的软件必须符合国际安全算法规范，`Shoulder` 默认使用美国国家安全局提倡的高性能、银行级安全的 `ECC 256`(
算法强度 > `RSA 3072`)

**国内特殊项目** 需要符合加密安全算法国产化需求，可通过配置切换为 `SM2` 国产加密算法。

### 🔒 安全说明

在客户端、服务端首次密钥协商后，哪怕受到网络抓包、中间人攻击、等极其恶劣的网络环境，仍能保障双向会话安全，就算使用世界上最牛逼的计算机长时间暴力破解单次请求加密密钥，也由于每次请求、响应密钥都不一致，无法还原上下文内容。
